...
template <class T>
float pagerankMonolithicSeqLoop(vector<T>& a, vector<T>& r, vector<T>& c, const vector<T>& f, const vector<int>& vfrom, const vector<int>& efrom, int i, int n, int N, T p, T E, int L, int EF) {
  int l = 0;
  float t = 0;
  T  c0 = (1-p)/N;  // non-teleport dead end handling!
  while (l<L) {
    t += pagerankCalculate(a, c, vfrom, efrom, i, n, c0);  // assume contribtions (c) is precalculated
